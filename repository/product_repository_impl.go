...
func (repo *productRepoImpl) FindSingleProduct(id int) (res model.ShowSingleProduct, err error) {
	...
	sqlStatement := "SELECT product_name,description,price,seller_id,created_at,updated_at FROM product WHERE id= ?"
	row := repo.sqlDb.QueryRowContext(ctx, sqlStatement, id)
	err = row.Scan(&res.ProductName, &res.Description, &res.Price, &res.SellerId, &res.CreatedAt, &res.UpdatedAt)
...
func (repo *productRepoImpl) FindAllProduct(filter string) (res []model.ShowProduct, err error) {
	ctx, cancel := config.NewMySqlContext()
	defer cancel()
	sqlStatement := "SELECT product.id,product_name, seller.name as seller_name, description, price, seller_id from product inner join seller on seller_id=seller.id " + filter
