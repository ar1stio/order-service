...
func (repo *orderRepoImpl) Create(req model.CreateOrder)(err error){
	...
	query := "INSERT INTO tbl_orders(buyer_id,seller_id,buyer_name,seller_name,delivery_source_address,delivery_destination_address,items,quantity,price,total_price,created_at) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
...
func (repo *orderRepoImpl) Update(req model.UpdateOrder)(err error){
	...
	query := "UPDATE tbl_orders SET buyer_id = ? ,seller_id = ? ,buyer_name = ? ,seller_name = ? ,delivery_source_address = ? ,delivery_destination_address = ? ,items = ? ,quantity = ? ,price = ? ,total_price = ? ,updated_at = ? WHERE order_uuid = ? "
	stmt, err := repo.sqlDb.PrepareContext(ctx, query)
	if err != nil {
		return err
	}	
...
func (repo *orderRepoImpl) Delivered(req model.Deliveredreq)(err error){
	...
	query := "UPDATE tbl_orders SET order_status = 1  WHERE id = ? "
	stmt, err := repo.sqlDb.PrepareContext(ctx, query)
	if err != nil {
		return err
	}	
...
func (repo *orderRepoImpl) ShowOrder(filter string) (res []model.ShowOrder, err error) {
	...
	sqlStatement := "SELECT id,buyer_id,seller_id,buyer_name,seller_name,delivery_source_address,delivery_destination_address,items,quantity,status,price,total_price,created_at " + filter
